...
def detection(img, a):
    ...
    with open(input_configuration_file) as js:
        input_data = json.load(js)
        input_temp = input_data[a]  # todo dodelat samostatne ulohy
        try:
        ...
        try:
            ...
            treshold, tresh = cv2.threshold(img, 110, 255, cv2.THRESH_BINARY)
            ...
            rows = colored_img.shape[0]
            circles = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 1, 28, param1=90, param2=13, minRadius=15,
                                       maxRadius=30)
